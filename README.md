<a href="https://keystonejs.com/">
  <img src="./.github/assets/banner.svg" alt="Keystone: The superpowered CMS for developers">
  </br>
  </br>
</a>

<p>
  <a aria-label="NPM version" href="">
    <img alt="" src="https://img.shields.io/github/v/release/ganobrega/quickreceitas?style=for-the-badge&labelColor=0869B8">
  </a>
  <a aria-label="Twitter Logo" href="">
    <img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white&labelColor=007BCE">
  </a>
  <a aria-label="Slack Logo" href="">
    <img src="https://img.shields.io/badge/Slack-8C0789?style=for-the-badge&logo=slack&logoColor=white&labelColor=4A154B">
  </a>
  <a aria-label="Website" href="https://quickreceitas.com.br">
    <img src="https://img.shields.io/badge/Website-2F6BFF.svg?style=for-the-badge&logo=data:image/svg+xml;base64,&labelColor=0737ad&locoColor=white&logoWidth=0">
  </a>
</p>

---

## Conteúdo

- [Uso e Documentação](#uso-e-documentação)
- [Versionamento](#versionamento)
- [Código de Conduta](#código-de-conduta)
- [Segurança](#segurança)
- [Licença](#licença)

## Uso e Documentação

O QuickReceitas é uma plataforma web que permite aos usuários:

- Encontrar receitas através de busca inteligente
- Compartilhar suas próprias receitas
- Salvar receitas favoritas
- Criar listas de compras
- Seguir outros cozinheiros

## Versionamento

QuickReceitas segue o padrão [semver](https://semver.org/).

## Código de Conduta

QuickReceitas segue o [Código de Conduta do Contribuidor](/CODE-OF-CONDUCT.md).

## Segurança

Para reportar vulnerabilidades, por favor consulte nossa [política de segurança](/SECURITY.md).

## Licença

Copyright (c) 2024 QuickReceitas. Licenciado sob a Licença MIT.